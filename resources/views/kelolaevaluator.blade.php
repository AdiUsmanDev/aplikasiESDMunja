      ...
      <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
        <nav>
          ...
          <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
            ...
            <li class="text-sm pl-2 capitalize leading-normal text-white before:float-left before:pr-2 before:text-white before:content-['/']" aria-current="page">Kelola Evaluator</li>
          </ol>
          <h6 class="mb-0 font-bold text-white capitalize">Kelola Evaluator</h6>
        </nav>
    ...
    <div class="overflow-x-auto bg-white shadow-lg rounded-xl p-4">
      <table class="min-w-full table-auto">
        <thead class="bg-blue-400 text-white">
          <tr>
            <th class="py-3 text-sm text-left px-4">No</th>
            <th class="py-3 px-4 text-sm text-left">Nama </th>
            <th class="py-3 px-4 text-sm text-center">Pangkat / Golongan</th>
            <th class="py-3 px-4 text-sm text-center">Email </th>
            <th class="py-3 px-4 text-sm text-center">Peran</th>
            <th class="py-3 px-4 text-sm text-center">Aksi</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-200">
        <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-gray-700">Kelola Evaluator</h2>
       <!-- Tombol di atas tabel -->
        <button onclick="openTambahEvaluatorModal()" class="bg-blue-500 text-white px-4 py-2 font-bold text-sm rounded-lg hover:bg-blue-600 transition">
        + Tambah Evaluator
        </button>
        </div>
        <!-- Contoh Baris 1 -->
        <tr class="hover:bg-blue-50">
            ...
            <span id="pangkat_1" class="inline-block px-2 py-1 text-xs bg-yellow-100 text-orange-800 text-xs font-semibold rounded">Pengatur Muda (II/a)</span>
            </td>
            <td class="py-3 px-4 text-xs text-center">budi.santoso@email.com</td> <!-- Email ditambahkan -->
            <td class="py-3 px-4 text-xs font-medium text-center" id="peran_1">Evaluator</td>
        ...
        <tr class="hover:bg-blue-50">
            ...
            <!-- Modal Popup -->
            <div id="modalTambahEvaluator" class="fixed inset-0 hidden items-center justify-center bg-black bg-opacity-50 z-50">
            <div class="bg-white w-full max-w-md p-6 rounded-lg shadow-lg">
                <h2 class="text-lg font-semibold mb-4">Form Tambah Evaluator</h2>
                <form id="formEvaluator" onsubmit="submitFormEvaluator(event)">
                <div class="mb-3">
                    <label class="block mb-1 text-sm font-medium">Nama</label>
                    <input type="text" id="nama" required class="w-full border border-gray-300 px-3 py-2 rounded" />
                </div>
                <div>
                      <label for="pangkat" class="block mb-1 text-sm font-medium">
                        Pangkat / Golongan
                      </label>
                      <select name="pangkat" id="pangkat" required
                ...
                <div class="mb-3">
                    ...
                    <input type="email" id="email" required class="w-full border border-gray-300 px-3 py-2 rounded" />
                </div>
                <div class="mb-3">
                    <label class="block mb-1 text-sm font-medium">Kata Sandi</label>
                    <input type="text" id="katasandi" required class="w-full border border-gray-300 px-3 py-2 rounded" />
                </div>
                <div class="flex justify-end gap-2">
                    <button type="button" onclick="closeTambahEvaluatorModal()" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">Batal</button>
            ...
            <script>
                ...
                function submitFormEvaluator(event) {
                    ...
                    <td class="py-3 px-4 text-xs font-medium">${nama}</td>
                    <td class="py-3 px-4 text-xs text-center">
                    <span id="pangkat" class="inline-block px-2 py-1 text-xs bg-yellow-100 text-orange-800 text-xs font-semibold rounded">${pangkat}</span>
                    </td>
                    <td class="py-3 px-4 text-xs text-center">${email}</td>
                    <td class="py-3 px-4 text-xs font-medium text-center">Evaluator</td>
                    <td class="py-3 px-4 text-center">
                        <button onclick="this.closest('tr').remove()" class="bg-green-600 hover:bg-green-700 text-white text-xs font-semibold px-4 py-2 rounded-lg transition">
                        Hapus
                        </button>
                    </td>
                    `;
